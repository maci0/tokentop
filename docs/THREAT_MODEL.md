...
| M5: Event field clamps (agent 64, model 128, note 512, kind 24 characters) + retention caps (512 events, 128 probes per snapshot) | memory pinning via oversized or numerous events (B1 DoS) | server.go:315-341; core.AgentHistoryLen / ProbeHistoryLen internal/core/core.go:8-21; collector.go:428-440,446-453 |
...
| M8: Engine response caps: 4 MiB JSON, 8 MiB text, 256-character error snippets | memory blowup and log flooding from hostile engines (B2 DoS/disclosure) | provider/provider.go:68,90; httperr/httperr.go:17,21-38 |
