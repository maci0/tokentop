...
| M19: Flag validation exits 2; set-but-invalid `TOKTOP_COLUMNS`/`TOKTOP_LINES` exit 2 under `--once` (and are named as ignored without it); non-TTY stdout aborts the live dashboard; malformed `~/.gauntlet/agents.json` warned instead of swallowed; unknown `TOKTOP_*` env warned; empty `--ingest` rejected; `--add` userinfo rejected | misconfiguration acting as silent security-relevant behavior change; empty ingest bind exposing every interface | main.go:159-215,547-565,775-818,857-925 |
