...
the page ships in two encodings under one URL, which one strong tag may not
span.

## Encoding

Browsers advertising gzip get a body that was compressed once when the isolate
started, not once per request; clients without gzip get the identity bytes.
Every response carries `Vary: Accept-Encoding`, so caches never hand a
...
current source: 6,719 bytes identity / 2,980 bytes gzipped - both inside the
~14 KB initial congestion window, so first paint needs a single round trip.
Anything pulling the page past one window (a script, a font, an image) needs a
better reason than decoration. The budget is pinned by a test, so drift fails
`bun test site/`; numbers above are re-measurable with it:
