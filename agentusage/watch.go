...
func (w *Watcher) Poll() Sample {
	...
	// file created seconds ago, and a review short enough to finish inside
	// rescanEvery would otherwise report nothing at all. The cost is one walk
	// per final read, not one per periodic poll: Run's ticker goes through
	// poll, which still reuses the cached listing.
	w.pollMu.Lock()
	w.cached = nil
	w.pollMu.Unlock()
	w.poll(nil)
